...
class HomeModel
{
    ...
    public static function getNotifications($member_id) {
        $result = Database::search("SELECT `notification_id`, `message`, `created_at`, `status` FROM `notification` WHERE `receiver_id` = '$member_id' ORDER BY `created_at` DESC LIMIT 5");
